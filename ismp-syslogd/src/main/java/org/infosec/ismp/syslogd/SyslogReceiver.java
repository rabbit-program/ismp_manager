...
class SyslogReceiver implements Runnable {
	...
	// private final SyslogdConfig m_syslogdConfig;
	 ...
	 * @param messageGroup
	 */
	SyslogReceiver(DatagramSocket sock,SyslogNodeManager syslogNodeManager) {
		m_stop = false;
		m_dgSock = sock;
		// m_syslogdConfig = config;
		...
		while (!m_stop) {
			...
			try {
				...
				Thread worker = new Thread(new SyslogConnection(pkt,
						m_syslogNodeManager.getAllSyslogNodes()),
						SyslogConnection.class.getSimpleName());
				worker.start();
...
//	public SyslogNodeManager getSyslogNodeManager() {
//		return m_syslogNodeManager;
//	}
//
