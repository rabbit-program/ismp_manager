...
public final class SyslogHandler {
	...
	private final SyslogNodeManager m_syslogNodeManager;

	static QueueManager queueManager = new QueueManager();

	public SyslogHandler(int port,SyslogNodeManager syslogNodeManager) {
		m_dgSock = null;
		m_dgPort = port;
...
//			System.out.println("---------------------------------");
//			System.out.println("m_ueiList is : "+m_UeiList.getUeiMatchCollection());
//			System.out.println("---------------------------------");
			
			m_dgSock = new DatagramSocket(m_dgPort);

			m_receiver = new SyslogReceiver(m_dgSock,m_syslogNodeManager);
