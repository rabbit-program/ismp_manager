 ...
 * 
 */
public class HostRefreshPanelUI {
	 ...
	 * @param data
	 */
	public HostRefreshPanelUI(NullFilter data) {
		if (data == null) {
		...
		Timer timer = new Timer(frequency, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							List<org.infosec.ismp.applet.manager.component.panel.view.network.NetworkStatus> networks = data.getHostNetworkStatusAll();
							...
							System.out.println("主机网络接口中间信息刷新发生错误！...");
							e.printStackTrace();
						}
					}
				});
		...
		Timer timer = new Timer(frequency, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							panel.setDiskCount(data.getHostHardDiskSize());
							panel.setUsedDisk(data.getHostHardDiskUsed());
						} catch (Exception e) {
							System.out.println("主机硬盘底部信息刷新发生错误！...");
							e.printStackTrace();
						}
					}
				});
		...
		Timer timer = new Timer(frequency, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							panel.setMemoryCount(data.getHostMemorySize());
							panel.setUsedMemeory(data.getHostMemoryUsed());
						} catch (Exception e) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							...
							panel.getDataBox().addElements(data.getHostCPUs());
						} catch (Exception e) {
							System.out.println("主机CPU底部信息刷新发生错误！...");
							e.printStackTrace();
						}
					}
				});
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							...
							List<Element> elements = data.getHostCPUs();
							...
							System.out.println("主机CPU中间信息刷新发生错误！...");
							e.printStackTrace();
						}
					}
				});
		...
		Timer timer = new Timer(frequency, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							panel.setValue((data.getHostMemoryUsed()/data.getHostMemorySize())*100);
						} catch (Exception e) {
			...
			public void actionPerformed(ActionEvent e) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							panel.clearFiles();
							for(FileElement f : data.getHostFileElements()) {
								panel.addFile(f);
							}
						} catch (Exception e) {
							System.out.println("主机硬盘中间信息刷新发生错误！...");
							e.printStackTrace();
						}
					}
				});
			...
			public void actionPerformed(ActionEvent e) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							panel.clearPortList();
							for(org.infosec.ismp.applet.manager.component.panel.view.network.NetworkStatus e : data.getHostNetworkStatusAll()) {
		...
		Timer timer = new Timer(frequency, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						try {
							List<org.infosec.ismp.applet.manager.component.panel.progress.Process> pro =data.getHostProcessAll();
							if(pro == null) return;
							...
							System.out.println("主机线程信息刷新发生错误！...");
							e.printStackTrace();
						}
					}
				});
