 ...
 * 
 */
public class HostDeviceInfoPanel {
	...
	public HostDeviceInfoPanel(final NodeModel node) {
		model = node;
	...
	class DataRefresh extends Thread {
		...
		public void run() {
			HostRefreshPanelUI refresh = new HostRefreshPanelUI(filter);
			//----------指定面板的刷新频率
			...
			info.routinePanel.setOs(model.getSystem());
		}
