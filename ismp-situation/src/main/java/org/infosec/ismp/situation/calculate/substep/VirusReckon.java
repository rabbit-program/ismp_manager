...
import org.infosec.ismp.situation.calculate.substep.callable.VirusCallable;
...
public class VirusReckon {
	...
	public void analyzeEvent(Event event) {
		int[] count;
		int x = event.getPriority();///获得事件的优先级
		String ip = event.getDestAddress();///获得目的IP
	...
	public void reckonExponential(Timestamp endtime, int index) {
		synchronized (map) {
			...
			completionService.submit(new VirusCallable(maps, index, endtime,lists));///提交计算病毒事件态势的任务
			map.clear();
			list.clear();
		}
	}
