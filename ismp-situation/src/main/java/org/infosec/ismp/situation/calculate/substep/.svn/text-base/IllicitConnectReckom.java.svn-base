...
import org.infosec.ismp.situation.calculate.substep.callable.IllicitConnectCallable;
...
public class IllicitConnectReckom {
	...
	public void analyzeEvent(Event event) {
		int[] count;
		int x = event.getPriority();///分析事件  参数是事件。
		String ip = event.getDestAddress();///分析事件  参数是事件。
	...
	public void reckonExponential(Timestamp endtime, int index) {
		synchronized (map) {
			...
			completionService.submit(new IllicitConnectCallable(maps, index, endtime,lists));///提交计算非法连接事件态势的任务
			map.clear();
			list.clear();
		}
	}
