...
import org.infosec.ismp.snmp.TrapIdentity;
import org.infosec.ismp.snmp.TrapInformation;
import org.infosec.ismp.snmp.TrapNotificationListener;
import org.infosec.ismp.snmp.TrapProcessor;
...
public class Snmp4JTrapNotifier implements CommandResponder {
	...
	private final TrapProcessorFactory m_trapProcessorFactory;
	private final TrapNotificationListener m_listener;
	...
	public static class Snmp4JV1TrapInformation extends TrapInformation {
		...
		private final PDUv1 m_pdu;

		protected Snmp4JV1TrapInformation(InetAddress agent, String community,
				PDUv1 pdu, TrapProcessor trapProcessor) {
		...
		protected TrapIdentity getTrapIdentity() {
			return new TrapIdentity(SnmpObjId.get(m_pdu.getEnterprise()
					.getValue()), m_pdu.getGenericTrap(),
	...
	public static class Snmp4JV2TrapInformation extends TrapInformation {
		 ...
		 * The received PDU
		 */
		private final PDU m_pdu;

		/**
		 * The name of the PDU's type
		 */
		private final String m_pduTypeString;
		 ...
		 */
		static final OID EXTREME_SNMP_SYSUPTIME_OID = new OID(
				".1.3.6.1.2.1.1.3");
		 ...
		 * 
		 */
		public Snmp4JV2TrapInformation(InetAddress agent, String community,
				PDU pdu, TrapProcessor trapProcessor) {
			super(agent, community, trapProcessor);
			m_pdu = pdu;
		...
		protected long getTimeStamp() {
			...
			if (log().isDebugEnabled()) {
				log().debug(
						"V2 " + m_pduTypeString + " first varbind value: "
								+ getVarBindAt(0).getVariable().toString());
			}

			switch (getVarBindAt(SNMP_SYSUPTIME_OID_INDEX).getVariable()
					.getSyntax()) {
			case SMIConstants.SYNTAX_TIMETICKS:
				...
				return ((TimeTicks) getVarBindAt(SNMP_SYSUPTIME_OID_INDEX)
						.getVariable()).getValue();
			case SMIConstants.SYNTAX_INTEGER32:
				log().debug(
						"V2 "
								+ m_pduTypeString
								+ " first varbind value is of type INTEGER, casting to TIMETICKS");
				return ((Integer32) getVarBindAt(SNMP_SYSUPTIME_OID_INDEX)
						.getVariable()).getValue();
			default:
		...
		@Override
		protected TrapIdentity getTrapIdentity() {
			OID snmpTrapOid = (OID) getVarBindAt(SNMP_TRAP_OID_INDEX)
					.getVariable();
			OID lastVarBindOid = getVarBindAt(getPduLength() - 1).getOid();
			Variable lastVarBindValue = getVarBindAt(getPduLength() - 1)
					.getVariable();
			return new TrapIdentity(SnmpObjId.get(snmpTrapOid.getValue()),
					SnmpObjId.get(lastVarBindOid.getValue()), new Snmp4JValue(
							lastVarBindValue));
		}
		...
		protected void validate() {
			...
			if (pduType != PDU.TRAP && pduType != PDU.INFORM) {
				throw new IllegalArgumentException(
						"Received not SNMPv2 Trap|Inform from host "
								+ getTrapAddress() + " PDU Type = "
								+ PDU.getTypeString(getPdu().getType()));
			}

			if (log().isDebugEnabled()) {
				log().debug(
						"V2 " + m_pduTypeString + " numVars or pdu length: "
								+ getPduLength());
			}
			...
			if (getPduLength() < 2) {
				throw new IllegalArgumentException(
						"V2 "
								...
								+ getPduLength() + ", needed at least 2");
			}
		...
		protected void processVarBindAt(int i) {
			if (i == 0) {
				log().debug(
						"Skipping processing of varbind "
								+ i
								+ ": it is sysuptime and the first varbind, and is not processed as a parm per RFC2089");
			} else if (i == 1) {
				log().debug(
						"Skipping processing of varbind "
								+ i
								+ ": it is the trap OID and the second varbind, and is not processed as a parm per RFC2089");
			} else {
				SnmpObjId name = SnmpObjId.get(getVarBindAt(i).getOid()
						.getValue());
				SnmpValue value = new Snmp4JValue(getVarBindAt(i).getVariable());
				processVarBind(name, value);
			}
	...
	public void processPdu(CommandResponderEvent e) {
			...
			if (command.getType() == PDU.INFORM) {
				...
				try {
					e.getMessageDispatcher().returnResponsePdu(
							...
							e.getSecurityLevel(), response,
							e.getMaxSizeResponsePDU(), ref, statusInformation);
					if (log().isDebugEnabled()) {
						log().debug(
								"Sent RESPONSE PDU to peer "
										+ addr
										+ " acknowledging receipt of INFORM (reqId="
										+ command.getRequestID() + ")");
					}
				} catch (MessageException ex) {
					log().error(
							"Error while sending RESPONSE PDU to peer " + addr
									+ ": " + ex.getMessage()
									+ "acknowledging receipt of INFORM (reqId="
									+ command.getRequestID() + ")");
				}
		...
		if (e.getPDU() instanceof PDUv1) {
			m_listener.trapReceived(new Snmp4JV1TrapInformation(addr
					.getInetAddress(), new String(e.getSecurityName()),
					(PDUv1) e.getPDU(), m_trapProcessorFactory
							.createTrapProcessor()));
		} else {
			m_listener.trapReceived(new Snmp4JV2TrapInformation(addr
					.getInetAddress(), new String(e.getSecurityName()), e
					.getPDU(), m_trapProcessorFactory.createTrapProcessor()));
		}
	}
