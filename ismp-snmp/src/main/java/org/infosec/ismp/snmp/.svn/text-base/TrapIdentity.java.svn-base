...
public class TrapIdentity {
	...
	public TrapIdentity(SnmpObjId snmpTrapOid, SnmpObjId lastVarBindOid,
		...
		if (TrapIdentity.GENERIC_TRAPS.contains(snmpTrapOid)) {
			...
			if (varBindName.equals(TrapIdentity.SNMP_TRAP_ENTERPRISE_ID)) {
				...
				setEnterpriseId(TrapIdentity.SNMP_TRAPS
						+ "."
						+ snmpTrapOidValue
		...
		} else // not standard trap
		{
			...
			int nextToLastIndex = snmpTrapOidValue.lastIndexOf(
					TrapIdentity.DOT_CHAR, lastIndex - 1);
	...
	public String toString() {
		return "[Generic=" + getGeneric() + ", Specific=" + getSpecific()
