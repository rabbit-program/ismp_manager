...
public class PollableNode extends PollableContainer {
			...
			if (m_owner != Thread.currentThread()) {
				...
				while (m_owner != null) {
					try {
						...
						throw new ThreadInterrupted("Lock for "
								+ PollableNode.this + " is unavailable", e);
					}
				}
		...
		public synchronized void obtain(long timeout) {
			...
			if (m_owner != Thread.currentThread()) {
				...
				while (m_owner != null) {
					...
					if (now >= endTime)
						throw new LockUnavailable("Unable to obtain lock for "
								+ PollableNode.this + " before timeout");
				}
				m_owner = Thread.currentThread();
				log.debug("Obtained lock for " + PollableNode.this);
			}
			...
			if (m_owner == Thread.currentThread()) {
				...
				if (m_obtainCount == 0) {
					ThreadCategory.getInstance(getClass()).debug(
							"Releasing lock for " + PollableNode.this);
					m_owner = null;
					notifyAll();
				}
			}
	...
	public PollableInterface createInterface(final InetAddress addr) {
		...
		Runnable r = new Runnable() {
			@Override
			public void run() {
				PollableInterface iface = new PollableInterface(
						PollableNode.this, addr);
				addMember(iface);
				retVal[0] = iface;
			}
		};
	 ...
	 * @return
	 */
	public PollableService createService(final InetAddress addr,
	...
	public Event createDownEvent(Date date) {
		return getContext().createEvent(EventConstants.NODE_DOWN_EVENT_UEI,
				getNodeId(), null, null, date, getStatus().getReason());
	}

	@Override
	public Event createUpEvent(Date date) {
		return getContext().createEvent(EventConstants.NODE_UP_EVENT_UEI,
				getNodeId(), null, null, date, getStatus().getReason());
	}
