...
import org.infosec.ismp.poller.pollable.PollableServiceConfig;
...
public class Poller extends AbstractServiceDaemon {
	...
	protected void onInit() {
		...
		log().debug(
				"init: serviceUnresponsive behavior: "
						+ (getPollerConfig().serviceUnresponsiveEnabled() ? "enabled"
								: "disabled"));
		...
		try {
			...
			log().fatal(
					"start: Failed to initialized the broadcast event receiver",
					t);
	...
	//
	// final String DB_CLOSE_OUTAGES_FOR_UNMANAGED_SERVICES =
	...
	// Updater svcUpdater = new Updater(m_dataSource,
	// DB_CLOSE_OUTAGES_FOR_UNMANAGED_SERVICES);
	// svcUpdater.execute(closeTime);
	//
	// final String DB_CLOSE_OUTAGES_FOR_UNMANAGED_INTERFACES =
	// "UPDATE outages set ifregainedservice = ? where outageid in (select outages.outageid from outages, ipinterface where ((outages.nodeid = ipinterface.nodeid) AND (outages.ipaddr = ipinterface.ipaddr) AND ((ipinterface.ismanaged = 'F') OR (ipinterface.ismanaged = 'U')) AND (outages.ifregainedservice IS NULL)))";
	// Updater ifUpdater = new Updater(m_dataSource,
	...
	// svcUpdater
	// .execute(closeTime, new Integer(eventId), new Integer(nodeId));
	// }
	//
	// public void closeOutagesForInterface(Date closeDate, int eventId,
	// int nodeId, String ipAddr) {
	// Timestamp closeTime = new Timestamp(closeDate.getTime());
	// final String DB_CLOSE_OUTAGES_FOR_IFACE =
	// "UPDATE outages set ifregainedservice = ?, svcRegainedEventId = ? where outages.nodeId = ? AND outages.ipAddr = ? AND (outages.ifregainedservice IS NULL)";
	// Updater svcUpdater = new Updater(m_dataSource,
	// DB_CLOSE_OUTAGES_FOR_IFACE);
	// svcUpdater.execute(closeTime, new Integer(eventId),
	// new Integer(nodeId), ipAddr);
	// }
	//
	// public void closeOutagesForService(Date closeDate, int eventId, int
	// nodeId,
	// String ipAddr, String serviceName) {
	// Timestamp closeTime = new Timestamp(closeDate.getTime());
	// final String DB_CLOSE_OUTAGES_FOR_SERVICE =
	// "UPDATE outages set ifregainedservice = ?, svcRegainedEventId = ? where outageid in (select outages.outageid from outages, service where outages.nodeid = ? AND outages.ipaddr = ? AND outages.serviceid = service.serviceId AND service.servicename = ? AND outages.ifregainedservice IS NULL)";
	// Updater svcUpdater = new Updater(m_dataSource,
	// DB_CLOSE_OUTAGES_FOR_SERVICE);
	// svcUpdater.execute(closeTime, new Integer(eventId),
	// new Integer(nodeId), ipAddr, serviceName);
	// }
	...
	private void createScheduler() {
		...
		try {
			...
			setScheduler(new LegacyScheduler("Poller", getPollerConfig()
					.getThreads()));
	...
	public void scheduleService(final int nodeId, final String nodeLabel,
			final String ipAddr, final String svcName) {
		...
		try {
			...
			Runnable r = new Runnable() {
				@Override
				public void run() {
					int matchCount = scheduleMatchingServices("ifServices.nodeId = "
							+ nodeId
							+ " AND ifServices.ipAddr = '"
							+ ipAddr
							+ "' AND service.serviceName = '" + svcName + "'");
					if (matchCount > 0) {
						svcNode.recalculateStatus();
						svcNode.processStatusChange(new Date());
					} else {
						log.warn("Attempt to schedule service " + nodeId + "/"
								+ ipAddr + "/" + svcName
								+ " found no active service");
					}
				}
			};
			node.withTreeLock(r);
			...
			log.error("Unable to schedule service " + nodeId + "/" + ipAddr
					+ "/" + svcName, e);
		}
	}
	...
	private int scheduleMatchingServices(String criteria) {
		// String sql =
		// "SELECT ifServices.nodeId AS nodeId, node.nodeLabel AS nodeLabel, ifServices.ipAddr AS ipAddr, "
		// +
		...
		Querier querier = new Querier(m_dataSource, sql) {
			...
			public void processRow(ResultSet rs) throws SQLException {
				if (scheduleService(rs.getInt("nodeId"),
						rs.getString("nodeLabel"), rs.getString("ipAddr"),
						rs.getString("serviceName"),
						"A".equals(rs.getString("status")),
						(Number) rs.getObject("svcLostEventId"),
						rs.getTimestamp("ifLostService"),
	...
	private void updateServiceStatus(int nodeId, String ipAddr,
	...
	private boolean scheduleService(int nodeId, String nodeLabel,
			String ipAddr, String serviceName, boolean active,
		...
		// if (pkg == null) {
		// if (active) {
		// log.warn("Active service "
		// + serviceName
		// + " on "
		// + ipAddr
		// + " not configured for any package. Marking as Not Polled.");
		// updateServiceStatus(nodeId, ipAddr, serviceName, "N");
		// }
		// return false;
		// } else if (!active) {
		// log.info("Active service " + serviceName + " on " + ipAddr
		// + " is now configured for any package. Marking as active.");
		// updateServiceStatus(nodeId, ipAddr, serviceName, "A");
		// }

		ServiceMonitor monitor = m_pollerConfig.getServiceMonitor(serviceName);
		if (monitor == null) {
			log.info("Could not find service monitor associated with service "
					+ serviceName);
			return false;
		}
		...
		try {
			...
			log.error("Could not convert " + ipAddr + " as an InetAddress "
					+ ipAddr);
			return false;
		}

		PollableService svc = getNetwork().createService(nodeId, nodeLabel,
				addr, serviceName);
		PollableServiceConfig pollConfig = new PollableServiceConfig(svc,
				m_pollerConfig, getScheduler());
		svc.setPollConfig(pollConfig);
		synchronized (svc) {
			if (svc.getSchedule() == null) {
				Schedule schedule = new Schedule(svc, pollConfig,
						getScheduler());
		...
		if (svcLostEventId == null)
			...
			PollEvent cause = new DbPollEvent(svcLostEventId.intValue(),
					svcLostUei, date);
	...
	// String serviceName, Package pkg) {
	//
	// if (pkg.getRemote()) {
	// log().debug(
	// "pollableServiceInPackage: this package: " + pkg.getName()
	// + ", is a remote monitor package.");
	// return false;
	// }
	//
	// if (!m_pollerConfig.serviceInPackageAndEnabled(serviceName, pkg))
	// return false;
	//
	...
	// public boolean packageIncludesIfAndSvc(Package pkg, String ipAddr,
	// String svcName) {
	// ThreadCategory log = ThreadCategory.getInstance(getClass());
	//
	// if (!getPollerConfig().serviceInPackageAndEnabled(svcName, pkg)) {
	// if (log.isDebugEnabled())
	// log.debug("packageIncludesIfAndSvc: address/service: "
	// + ipAddr
	// + "/"
	// + svcName
	...
	// if (!getPollerConfig().interfaceInPackage(ipAddr, pkg)) {
	// if (log.isDebugEnabled())
	// log.debug("packageIncludesIfAndSvc: interface "
	// + ipAddr + " gained service " + svcName
