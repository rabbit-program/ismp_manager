...
function Boxy(element, options) {
    ...
    jQuery.each(['sve'], function() {
    	if (this in options) {
    		 jQuery.data(ebox[0], 'xboxy', options['sve']);	
    	}
    });
...
jQuery.extend(Boxy, {
    ...
    //---
    shload: function(url, options,method) {
        options = options || {};
        var ajax = {
            url: url, type: 'GET', dataType: 'html', cache: false, success: function(html) {
                html = jQuery(html);
    ...
    getEleValue:function(ele){
        var p = jQuery(ele).parents('.boxy-wrapper');
        return p.length ? jQuery.data(p[0], 'xboxy') : null;
    },
