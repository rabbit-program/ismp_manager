$.fn.simpleTree = function(opt){
	return this.each(function(){
				...
				if(url && url.indexOf('url:'))
				{
					...
					$.ajax({
						...
						success: function(responce){
							...
							var str="";
							$(responce).find('SH').each(function(index){
							   	var name=$(this).children("name").text();
							   	var id=$(this).children("id").text();
							   	var isleaf=$(this).children("isleaf").text();
							   	if(jQuery.trim(isleaf)==0){
							   		str+="<li id='"+parentId+index+"'>";
								   	str+="<span shid="+id+" isleaf="+isleaf+">"+name+"</span>";
								   	str+="<ul class='ajax'>";
									str+="<li id='"+parentId+index+index+"'>{url:doManinTree.do?method=getDoMain&sid="+id+"}</li>"; 
									str+="</ul></li>";	
								}else{
									str+="<li id='"+parentId+index+"'><span shid="+id+" isleaf="+isleaf+">"+name+"</span></li>";
								}
						   	});
						  //alert(str);
							node.html(str);
		...
		TREE.setTreeNodes = function(obj, useParent){
			...
			.bind('selectstart', function() {
				...
				if(TREE.option.drag)
				{
					...
					$("<img>").attr({id	: "tree_plus",src	: "/manager-web/img/VPM/pm/plus.gif"}).css({width: "7px",display: "block",position: "absolute",left	: "5px",top: "5px", display:'none'}).appendTo("body");
		...
		TREE.setTrigger = function(node){
			$('>span',node).before('<img class="trigger" src="/manager-web/img/VPM/pm/spacer.gif" border=0>');
		...
		TREE.dragStart = function(event){
			...
			if(mousePressed)
			{
				...
				if(event.target.tagName.toLowerCase()=='span' && $.inArray(event.target.className, Array('text','active','trigger'))!= -1)
				{
					...
					{
						if(isrc.indexOf('minus')!=-1)$("#tree_plus").attr('src','/manager-web/img/VPM/pm/plus.gif');
					...
					{
						mouseMoved = false;
						$("#tree_plus").attr('src','/manager-web/img/VPM/pm/minus.gif');
						$("#tree_plus").css({"left": screenScroll.x, "top": screenScroll.y}).show();
						...
						*/
						TREE.setAjaxNodes(ajaxChild,parent.id, function(){
							parent.className = parent.className.replace('close','open');
							mouseMoved = true;
							$("#tree_plus").attr('src','/manager-web/img/VPM/pm/plus.gif');
							$("#tree_plus").css({"left": screenScroll.x, "top": screenScroll.y}).show();
						});
