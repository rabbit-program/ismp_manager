...
//查看告警规则详细内容
function openAlertRuleDetail(url){
    window.open (''+url+'','alertDetailWindow','height=255,width=950,top=300,left=400,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no') ;
    
}
//查看用户详细类容
function userDetail(url){
	window.open (''+url+'','newwindow','height=300,width=950,top=300,left=400,toolbar=no,menubar=no,scrollbars=no,resizable=yes,location=no,status=no') ;
}
//删除物理位置节点
function delParent(parentId){
	location.href="location.do?method=getChialNode&del=1&parentId="+parentId;
}
