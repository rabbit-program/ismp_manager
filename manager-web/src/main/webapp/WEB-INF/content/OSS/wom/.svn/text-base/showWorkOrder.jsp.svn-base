    ...
    <head>
        ...
        <script type="text/javaScript">
            function toPage(curpage) {
	            ...
	            document.respShow.action="${ctx}/ismp/domain/local/oss/workOrder.do?method=showWorkOrder&isAll="+isAll+"&currPage="+curpage;
	            respShow.submit(); 
	        }
            function clean()
            {
            ...
            function OnClose(id){
                ...
                var r = confirm("确认关闭")
                if (r==true) {
                  window.location.href ="${ctx}/ismp/domain/local/oss/workOrder.do?method=OnClose&isAll="+isAll+"&currPage="+currPage+"&id="+id;
                }
            }

            function OverWorkOrder(id){
                var currPage=document.getElementById("currPage").value;
                var isAll = document.getElementById("isAll").value ;
                var r = confirm("确认结单")
                if (r==true) {
                  window.location.href ="${ctx}/ismp/domain/local/oss/workOrder.do?method=OverWorkOrder&isAll="+isAll+"&currPage="+currPage+"&id="+id;
                }
            }
            ...
            function back(){
                var isAll = document.getElementById("isAll").value ;
                window.location.href ="${ctx}/ismp/domain/local/oss/clientQuestion.do?method=showQuestion&isAll="+isAll;
            }
</script>
</head>
<body>
<form action="${ctx}/ismp/domain/local/oss/workOrder.do?method=showQuestion&isAll=${isAll}" id="respShow" name="respShow" method="post">
              ...
              <%@ include file="/WEB-INF/content/OSS/wom/comm/topmenu.jsp"%>
                 <div class="greenborder pad3 overf">
                    ...
                    <div class="greenborder greenback overf pad3 Height_a" >
                        <p><a class="R6 R7" href="#"  onclick="MM_openBrWindow('${ctx}/ismp/domain/local/oss/workOrder.do?method=preAddWorkOrder&isAll=${isAll}','新建工单','scrollbars=yes,width=480,height=580')" >新建工单</a></p>
                    </div>
                    ...
                    <h2>工单列表(表格现有排序功能,点击带箭头单元格)</h2>
                    <div id="data" class="greenborder overf pad1 " >
                        <table id="senfe" sortcol="-1">
                            <thead>
                                <tr>
                                    <th onclick="sortTable('senfe',0);" style="cursor:pointer">工单ID<span class="webdings">6</span></th>
                                    <th>所属问题名称</th>
                                    <th>处理人</th>
                                    <th>紧急程度</th>
                                    <th>工单状态</th>
                            ...
                            <tbody>
                                 <c:forEach var="workOrder" items="${workOrderList}" varStatus="status">
                                    <tr>
                                        <td> ${workOrder.id } </td>
                                        <td> ${workOrder.question.name} </td>
                                        <td> ${workOrder.operator.name} </td>
                                        
                                        <td> <c:choose><c:when test="${workOrder.level==1}">高</c:when>
                                        <c:when test="${workOrder.level==2}">中</c:when><c:otherwise>低</c:otherwise></c:choose> </td>
                                        <td> <c:choose><c:when test="${workOrder.state==1}">未处理</c:when>
										<c:when test="${workOrder.state==2}">处理中</c:when>
										<c:when test="${workOrder.state==3}">已处理</c:when>
										<c:when test="${workOrder.state==4}">已关闭</c:when>
                                        <c:otherwise>未知</c:otherwise></c:choose> </td>
										<td> ${workOrder.clientValue } </td>
                                        <td> ${workOrder.createTime } </td>
                                        <td> ${workOrder.endTime } </td>
                                        <td>
                                          <c:if test="${workOrder.state!=4}">
                                            <c:if test="${workOrder.state!=3}">
                                            <a href="#" onclick="OnClose('${workOrder.id }')" class="R6" >关闭</a>
                                            </c:if>
                                            <c:if test="${workOrder.state==2}">
                                            <a class="R6 R7" href="#" onclick="OverWorkOrder('${workOrder.id }')">结单</a>
											</c:if>
										  </c:if>                                           
										  <a class="R6 R7" href="#" onclick="MM_openBrWindow('${ctx}/ismp/domain/local/oss/workOrder.do?method=lookWorkOrder&id=${workOrder.id}','查看','scrollbars=yes,width=480,height=550')">查看</a>
                                        </td>
                                    </tr>
                                </c:forEach>
