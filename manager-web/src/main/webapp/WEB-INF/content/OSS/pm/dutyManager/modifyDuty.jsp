...
<html>
    <head>
        ...
        <title>运维支撑--值班管理--排班管理--修改排班详细记录</title>
...
$(function(){
    $.post("${ctx}/ismp/domain/local/vpm/pm/doManinTree.do?method=getDoMainAll",function(response){
        $(response).find('SH').each(function(index,evendate){
            $("<option "+($.trim($(this).children('id').text()) == $.trim("<bean:write name='dutySchedule' property='domain.id'/>") ? "selected='selected'" : " ")+" value='" + $(this).children('id').text() + "'>" + $(this).children('name').text() + "</option>").appendTo($("#doselect"));
        });    
   },'html');
...
<div id="contant" class="mainbg" style="width: 700px;">
  ...
  <div id="data" class="greenborder overf pad1" >
       <form action="${ctx}/ismp/oss/pm/dsinfo.do?method=dsUpdate" method="post" name="f1" id="f1">
           <input id='rostersList' name='dutyId' type='hidden' value='<bean:write name='dutySchedule' property='id' />' />
          <table width="100%" id="senfe" sortcol="-1">
              ...
              <tr>
               ...
               <td colspan="6">
                    <input type="text" size="8" id="startTime" name="startTime" class="input" readonly="readonly" value="<bean:write name='dutySchedule' property='startTime'/>" onclick="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd HH:mm:ss'})"/> 
                    —
                    <input type="text" size="8" id="endTime" name="endTime" class="input" readonly="readonly" value="<bean:write name='dutySchedule' property='endTime'/>" onclick="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
                </td>
              </tr>
              <tr>
 ...
 <input type="button" onclick="addRoster(document.getElementById('rosterShow'),document.getElementById('select4'))" value="添加"/>
                    <div style="padding-top: 15px;padding-bottom: 5px;float: left;">
                        <div style="border: 1px solid #A4CE31;float: left;min-width: 500px;max-width: 546px;">
                            <div id="rosterShow" style="line-height: 20px;">
                                <input id="rosterNum" name="rosterNum" type="hidden" value="<bean:size name='dutySchedule' id='xx' property='roster'/>"/>
                             <logic:iterate id="ds" name="dutySchedule" property="roster" scope="request" indexId="len">
                                <div id="<bean:write name='ds' property='id' />" style="float: left; padding-left: 10px;">
                                    <span title="<bean:write name='ds' property='name' />" style="float: left; width: 60px;"><bean:write name='ds' property='name' /></span>
                                    <input id='rostersList' name='rostersList' type='hidden' value='<bean:write name='ds' property='id' />' />
                                    <a href="javascript:delRoster('<bean:write name='ds' property='id' />')" style="float: left; padding-top: 3px;" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image03','','${ctx}/img/comm/other/110.gif',1)">
              ...
              <tr>
                ...
                <td colspan="5"><textarea name="textfield" cols="100%" rows="5" id="textfield"><bean:write name='dutySchedule' property='task'/></textarea></td>
              </tr>
          </table>
    ...
    <ul id="page">
        <li><a href="#" onclick="rclose()" class="R6 R7 boxy" >确认</a></li>
        <li><a href="#" onclick="formreset()" class="R6 R7 boxy" >重置</a></li>
  </ul>
  </div>
  </div>
    </form>
...
<script type="text/javascript">
      ...
      <logic:iterate id="ds" name="dutySchedule" property="roster" scope="request" >
         rosterArray.push(<bean:write name='ds' property='id'/>); 
      </logic:iterate>
</script>
</body>
