...
<body>
<html:form action="/ismp/domain/local/oss/updateOssKB.do?method=update">
    <div id="data" class="pad1 ">
        
        <table>
            <tr>
                <th>知识库名称<span class="alert">*</span></th>
                <td><input size="8" id="name" name="name" value="${ossKB.name}" /></td>
              <!--<th>所属单位<span class="alert">*</span></th>
                <td>
                    <select id="dmid" name="dmid">
                        <c:forEach items="${udl}" var="ud">
                            <option value="${ud.id}"<c:if test="${ud.id == ossKB.domain.id}">selected='selected'</c:if>>${ud.domainName}</option>
                        </c:forEach>
                    </select>-->
                </td>
            </tr>
            <tr>
                ...
                <td colspan="3"><textarea cols="60" rows="5" name="file_content" id="file_content">${ossKB.file_content}</textarea></td>
            </tr>
            <tr>
                <th>发布人员<span class="alert">*</span></th>
                <td colspan="3"><textarea id="issuer" name="issuer" cols="60" rows="5" >${ossKB.issuer}</textarea></td>
            </tr>
            <tr>
                <th>知识库编号<span class="alert">*</span></th>
                <td colspan="3"><textarea id="sn" name="sn" cols="60" rows="5">${ossKB.sn}</textarea></td>
            </tr>
            <tr>
                <th>知识库描述<span class="alert">*</span></th>
                <td colspan="3"><textarea id="remark" name="remark" cols="60" rows="5">${ossKB.remark}</textarea></td>
            </tr>
        </table>
