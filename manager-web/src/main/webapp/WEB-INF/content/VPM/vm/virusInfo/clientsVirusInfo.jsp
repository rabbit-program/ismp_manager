...
<div id="contant" class="mainbg">
  ...
  <div class="contant">
    ...
    <table>
        <tr>
          <td width="200"><div class="greenback overf pad3" style="width:200px; height:500px; overflow:auto; margin:0;">
           <jsp:include page="../comm/leftFrameTree.jsp"></jsp:include>
           </div></td>
          <td valign="top">         
    ...
    <h2 class="martop10">客户端基本信息</h2>
    <div class="greenborder greenback overf pad3" >客户端名称：<b class="font_tip marright10">${vc.name}</b> 客户端IP：<b class="font_tip marright10">${vc.clientIP}</b> 关联资产：<b class="font_tip marright10">${vc.assetDevice.name}</b> 关联部门：<b class="font_tip marright10">${vc.department.domainName}</b></div>
        <table class="martop8">
  <tr>
    <td width="50%" style="border-right:1px solid #a4ce31;"> <%@ include file="/WEB-INF/content/VPM/vm/virusInfo/virusTopN.jsp"%></td>
  </tr>
</table>
  ...
  <tr>
    ...
    <th> 操 作 </th>
        ...
        <c:forEach var="alert" items="${alertList}">
            <tr>
              <td><div>${alert.virusClients.name}</div></td>
              <td><div>${alert.virusClients.clientIP}</div></td>
              <td><div>${alert.virus.name}</div></td>
