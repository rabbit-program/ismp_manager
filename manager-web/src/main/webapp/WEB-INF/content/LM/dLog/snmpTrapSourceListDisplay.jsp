...
function validate_SnmpTrapSourceQuery(){
...
<div id="contant" class="mainbg">
  ...
  <div class="contant">
   <div class="overf h20 martop10">
      ...
      <li><a  class="mbacka" href="snmpTrapAction.do?method=getSnmpTrapSource"><span class="menus11">Trap</span></a></li>
...
<form action="snmpTrapAction.do?method=getSnmpTrapSource" method="post" onsubmit="return validate_SnmpTrapSourceQuery();">
...
&nbsp;&nbsp;&nbsp;
          ...
          <table id="senfe" sortcol="-1">
			...
			<c:forEach items="${pageResult.pageResult}" var="logSource" varStatus="status">
				<tr>
					...
					<td><a href="snmpTrapDivSourceAction.do?method=snmpTrapGetDevice&device=${logSource.sourceType.handleOrParserType.webHandleClass }&logsourcelogo=${logSource.logSourceseQuence }" class="R6 R7">查看日志源</a></td>
				</tr>
			</c:forEach>
            </tbody>
          </table>
      ...
      <ul id="page">
        <li><a class="first" href="snmpTrapAction.do?method=getSnmpTrapSource&type=page"></a></li>

		<c:if test="${pageResult.pageNo-1 >= 1}">
        	<li><a class="pre" href="snmpTrapAction.do?method=getSnmpTrapSource&type=page&pageNo=${pageResult.pageNo-1 }"></a></li>
		</c:if>
		<c:if test="${pageResult.pageNo-1 == 0}">
        	<li><a class="pre" href="snmpTrapAction.do?method=getSnmpTrapSource&type=page"></a></li>
		</c:if>

		<c:if test="${pageResult.pageNo+1 < pageResult.pageMaxSum}">
        	<li><a class="next" href="snmpTrapAction.do?method=getSnmpTrapSource&type=page&pageNo=${pageResult.pageNo+1 }"></a></li>
		</c:if>
		<c:if test="${pageResult.pageNo+1 >= pageResult.pageMaxSum}">
        	<li><a class="next" href="snmpTrapAction.do?method=getSnmpTrapSource&type=page&pageNo=${pageResult.pageMaxSum }"></a></li>
		</c:if>

			<li><a class="last" href="snmpTrapAction.do?method=getSnmpTrapSource&type=page&pageNo=${pageResult.pageMaxSum }"></a></li>
        <li>共 ${pageResult.pageMaxSum } 页 跳至
          <input type="text" size="2" class="input" name="mark"/>
          &nbsp; </li>
        <li><a class="R6" href="javascript:goPage('${pageResult.pageMaxSum }','snmpTrapAction.do?method=getSnmpTrapSource&type=page');">GO</a></li>
      </ul>
      <br />
      <br />
      <br />
