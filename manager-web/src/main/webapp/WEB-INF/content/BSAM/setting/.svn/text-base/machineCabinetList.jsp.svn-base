...
<script type="text/javascript">
  function toPage(currPage) {
      ...
      if("number" == typeof(currPage)){
          ...
          window.location.href="${ctx}/ismp/domain/local/bsam/machineCabinetAction.do?method=getMachineCabinetList&currPage="+currPage+"&isAll="+isAll;
      }else {
  ...
  function modify(id){
      ...
      var isAll = document.getElementById("isAll").value;
	  window.location.href="${ctx}/ismp/domain/local/bsam/machineCabinetAction.do?method=machineCabinetInput&id="+id+"&currPage="+currPage+"&isAll="+isAll;
  }

  function add(){
	  var currPage=document.getElementById("currPage").value;
      var isAll = document.getElementById("isAll").value;
      window.location.href="${ctx}/ismp/domain/local/bsam/machineCabinetAction.do?method=machineCabinetInput&currPage="+currPage+"&isAll="+isAll;
  }
  ...
  function dele(id){
      if(window.confirm("是否删除？")){
    	  var currPage=document.getElementById("currPage").value;
          var isAll = document.getElementById("isAll").value;
    	  window.location.href="${ctx}/ismp/domain/local/bsam/machineCabinetAction.do?method=machineCabinetDelete&id="+id+"&isAll="+isAll+"&currPage="+currPage;
      }
  }
 </script>
</head>
...
<div id="contant" class="mainbg">
    ...
    <div class="contant" >
        <div class="overf h20 martop10">
          ...
          <li >
               <a class="mbacka" href="${ctx}/ismp/domain/local/bsam/machineCabinetAction.do?method=getMachineCabinetList<sec:authorize ifAllGranted='ROLE_AdminAll'>&isAll=1</sec:authorize>">
        ...
        <form name="f1" id="f1" action="/ismp/domain/local/bsam/machineCabinetAction.do?method=getMachineCabinetList" method="post">
...
<!--           <input type="hidden" id="domainArr" value="${domainArr}" />-->
           ...
           <div id="data" class="greenborder overf pad1" >
                <table id="titleName" sortcol="-1">
                    <thead>
                      <tr>
                        <th width="25%" style="cursor:pointer" onclick="sortTable('senfe',0);">机柜名</th>
					    <th width="25%">所在机房</th>
					    <th width="25%">描述</th>
					    <th width="25%">操作</th>
                      </tr>
                ...
                <table id="senfe" name="machineCabinetTable" sortcol="-1" border="0" cellpadding="0" cellspacing="1">
                    <tbody>
                        <logic:notEmpty name="machineCabinetList">
                            <logic:iterate id="machineCabinet" name="machineCabinetList" indexId="i">
                            <tr>
                              <td width="25%">${machineCabinet.machineCabinetName }</td>
							  <td width="25%">${machineCabinet.machineRoom.machineRoomName}</td>
							  <td width="25%">${machineCabinet.description }</td>
                              <td width="25%">
                                <input type="button" class="commonbtn" value="修改" hidefocus="true" onclick="modify('${machineCabinet.id}')" /> 
                                <input type="button" class="commonbtn" value="删除" hidefocus="true" onclick="dele('${machineCabinet.id}')" />
                              </td>
                            </tr>
                            </logic:iterate>
                         </logic:notEmpty>    
                         <logic:empty name="machineCabinetList">
                            <tr>
                                <td colspan="6">没有找到机柜！</td>
                            </tr>
                         </logic:empty>
                    </tbody>
                </table>
