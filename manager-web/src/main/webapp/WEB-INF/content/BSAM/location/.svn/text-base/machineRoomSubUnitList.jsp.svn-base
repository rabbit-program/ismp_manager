	    ...
	    <div class="overf h20 martop10">
	      <li id="m">
	           <a class="mbacka" href="${ctx}/ismp/domain/local/bsam/situationAction.do?method=getSubUnitList&type=${type}&id=${id}">
	                <span class="menus10">机房子单位列表</span>
	           </a>
	      </li>
	    </div>
...
<!--                                </span>-->
                                <c:if test="${subUnit.type == 'JiGui'}">
                                    <a href="#" 
                                        onclick="subOnclick('${subUnit.id}','${subUnit.name}','2')"
                                        ondblclick="getSubSituation('JiGui',${subUnit.id})" title="${subUnit.name}" id="${subUnit.name}" >
                                        <img src="${ctx}/img/BSAM/icon_02.gif"  id="${subUnit.name}alert"/>
                                    </a>
                                </c:if>
...
<!--                        <input id="currPage" name="currPage" type="text" size="2" class="input" value="${currPage}"/>&nbsp;-->
                    ...
                    <li><input type="button" class="R6 R7" value="返回" hidefocus="Add" onclick="javascript:history.go(-1)"/></li>
                </ul>
                <br/><br/><br/>
            <!--Applet页面-->
